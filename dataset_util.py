...
def save_wsi_thumbnail_mask(filename):
    ...
    thumbnail_size = min(2000.0, max(l_dims[0]))
...
def get_random_wsi_patch(filename, patch_size=256, downsample=4, augment=0):
    ...
    try:
        print(f"Trying 1: {filename.decode()}")
        wsi = openslide.OpenSlide(filename.decode())
        print(f"Success 1: {filename.decode()}")
    except:
        print(f"Trying 2: {filename}")
        wsi = openslide.OpenSlide(filename)
        print(f"Success 2: {filename}")
        ...
        while True:
            ...
            try:
                ...
                region = wsi.read_region(
                    (int(x_start * level_downsample), int(y_start * level_downsample)),
                    level,
                ...
                def PiecewiseAffine(img, points=8):
                    ...
                    for i in list(range(points))[1:-1]:
                        dst_rows[i::points] += np.random.normal(
                            loc=0, scale=rows / (points * 10), size=dst_rows[i::points].shape
                        )
                    dst_cols = np.zeros(src[:, 0].shape) + src[:, 0]
                    dst_cols[points:-points] += np.random.normal(
                        loc=0, scale=rows / (points * 10), size=dst_cols[points:-points].shape
                    )
                ...
                if np.random.random() < augment:
                    # augment image
                    region = (region / 255.0).astype(np.float64)
                    region = colorshift(region)
                    region = PiecewiseAffine(region)
                    region = np.uint8(region * 255.0)
                    # unpad
